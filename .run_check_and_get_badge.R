...
# - r-cmd-check-badge.svg (badge)
# - R_CMD_CHECK.txt (errors, warnings and notes from the check() call)
...
cat("* ERRORS\n\n", file = outFile)
invisible(lapply(output$errors, function(e) {
    cat("** ", e, "\n\n", file = outFile)
}))
cat("* WARNINGS\n\n", file = outFile)
invisible(lapply(output$warnings, function(w) {
    cat("** ", w, "\n\n", file = outFile)
}))
cat("* NOTES\n\n", file = outFile)
invisible(lapply(output$notes, function(n) {
    cat("** ", n, "\n\n", file = outFile)
}))
close(outFile)
